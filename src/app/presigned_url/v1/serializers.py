...
from app.common.storages import PublicMediaStorage
from app.presigned_url.utils import FIELD_CHOICES


class PresignedSerializer(serializers.Serializer):
    field_choice = serializers.ChoiceField(choices=FIELD_CHOICES, write_only=True, required=False)
    ...
    def validate(self, attrs):
        if attrs.get("field_choice"):
            model, field_name = self.get_field_info(attrs["field_choice"])
            ...
            response = PublicMediaStorage().generate_presigned_post(
                f"upload/{attrs['file_name']}", attrs["is_download"]
            )
            attrs.update(response)
        return attrs
